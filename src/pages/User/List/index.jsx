        ...
        <thead className="bg-gray-300">
          <tr className="h-16 border border-gray-100 rounded">
            <th className="text-left pl-5">
              <Text size="text-lg">{LANG.USERS_LIST.USERS_TABLE.USERS_NAME}</Text>
            </th>
            <th className="text-left w-32 md:w-52">
              <Text size="text-lg">{LANG.USERS_LIST.USERS_TABLE.USERS_EMAIL}</Text>
            </th>
            <th className="text-center w-32 md:w-72">
              <Text size="text-lg">{LANG.USERS_LIST.USERS_TABLE.USERS_ROLE}</Text>
            </th>
            <th className="text-center w-32 md:w-72">
              <Text size="text-lg">{LANG.USERS_LIST.USERS_TABLE.USERS_ACTIONS}</Text>
            </th>
          </tr>
        </thead>
        <tbody>
          {
            usersList?.length ? usersList.map((users) => (
              <tr key={users?.id} className="h-16 border border-gray-100 rounded">
                <td className="pl-5">
                  <Text>{users?.name}</Text>
                </td>
                <td>
                  <div className="flex items-center">
                    <Text>{new Date(users?.updatedAt).toLocaleString('pt-BR')}</Text>
                  </div>
                </td>
                <td>
                  <div className="flex flex-col md:flex-row gap-2">
                    <ColorButton color="blue" onClick={() => updateUsers(users?.id)}>
                      <Text>{LANG.USERS_LIST.ACTIONS.EDIT}</Text>
                    </ColorButton>
                    <ColorButton color="red" onClick={() => deleteUsers(users?.id)}>
